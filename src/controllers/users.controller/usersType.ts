...
const createUserType = asyncHandler(async (req, res, next) => {
    try {
        let jsonData  = req.body;
    
        jsonData.createdDt = dayjs().format('D-MMM-YYYY HH:mm');
        jsonData.modifiedDt = dayjs().format('D-MMM-YYYY HH:mm');
        let createResponse =  await userTypeServices.createUserType(jsonData);
...
const getUserType = asyncHandler(async (req, res, next) => {
    try {
        let {
            pageno= '1',
            perpage='10',
        ...
        let createResponse =  await userTypeServices.getUserTypes(getRoleQuery);
...
const deleteUserType  = asyncHandler(async (req, res, next) =>{
    try {
        let {
            userTypeId, 
            id
        } = req.query;
        let deleteResponse = await userTypeServices.deleteUserType(userTypeId, id);
