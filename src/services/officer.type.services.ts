...
const createOfficerType = async (roleBody: officerType) => {
  try {
    let db = await clientPromise;
    let role = await db.collection('officertypes');
    roleBody.officerTypeId = (await role.count()) + 1

    let insertedDoc = await role.updateOne(
      { '$and': [{ officerTypeNameEn: roleBody?.officerTypeNameEn }, { createdBy: roleBody?.createdBy }] },
...
const getOfficerType = async (query: any) => {
  try {
    let db = await clientPromise;
    let departments = await db.collection('officertypes');
    ...
    if (pagData && Array.isArray(pagData?.data)) {
      ...
      return {
        ...
        error: false,
        code: 400,
        msg: 'Officer type collection is empty'
      }
    }
  } catch (e: any) {
    return {
...
const deleteOfficerType = async (officerTypeId: string | any, id: string | any) => {
  try {
    let db = await clientPromise;
    let role = await db.collection('officertypes');
    console.log()
    // create a filter for a movie to update
    const filter = { '$and': [{ createdBy: id }, { 'officerTypeId': parseInt(officerTypeId) }] };
    ...
    if (docs && docs?.modifiedCount == 1) {
      ...
      let msg_ = docs?.matchedCount == 0 ? `no document matched give admin id ${id} and department ${officerTypeId}` : docs?.modifiedCount == 0 ? `content not modfied` : 'Not modified'
      return {
        data: [],
        error: false,
        code: 400,
