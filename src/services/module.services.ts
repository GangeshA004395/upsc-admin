...
const createModule = async (roleBody: moduleType) => {
  try {
    let db = await clientPromise;
    let role = await db.collection('modules');
    roleBody.moduleId = (await role.count()) + 1

    let insertedDoc = await role.updateOne(
      { '$and': [{ roleNameEn: roleBody?.moduleNameEn }, { createdBy: roleBody?.createdBy }] },
...
const getModule = async (query: any) => {
  try {
    let db = await clientPromise;
    let role = await db.collection('modules');
...
const deleteModule = async (moduleId: string | any, id: string | any) => {
  try {
    let db = await clientPromise;
    let role = await db.collection('modules');
    // create a filter for a movie to update
    const filter = { '$and': [{ createdBy: id }, { 'moduleId': parseInt(moduleId) }] };
    ...
    if (docs && docs?.modifiedCount == 1) {
      ...
      let msg_ = docs?.matchedCount == 0 ? `no document matched give admin id ${id} and moduleId ${moduleId}` : docs?.modifiedCount == 0 ? `content not modfied` : 'Not modified'
      return {
        data: [],
        error: false,
        code: 400,
